...
from rcnn.core.loader import AnchorLoaderFPN
...
def train_net(args, ctx, pretrained, epoch, prefix, begin_epoch, end_epoch,
    ...
    if args.resume:
        ...
        arg_params['mask_pred1_weight'] = mx.random.normal(0, 0.01, shape=arg_shape_dict['mask_pred1_weight'])
        arg_params['mask_pred2_weight'] = mx.random.normal(0, 0.01, shape=arg_shape_dict['mask_pred2_weight'])
    ...
    mask_metric = metric.MaskLossMetric()
    eval_metrics = mx.metric.CompositeEvalMetric()
    for child_metric in [rpn_eval_metric, rpn_cls_metric, rpn_bbox_metric, eval_metric, cls_metric, bbox_metric, mask_metric]:
